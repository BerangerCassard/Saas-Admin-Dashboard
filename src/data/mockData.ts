...
const companies = ['TechCorp', 'InnoSoft', 'DataFlow', 'CloudSync', 'NextGen', 'PixelWorks', 'CodeBase', 'DevHub', 'AppForge', 'ByteStream', 'LogicLab', 'SyncFlow', 'Quantum', 'Nexus', 'Vertex', 'Zenith', 'Horizon', 'Catalyst', 'Luminary', 'Pinnacle'];

export const generateCustomers = (): Customer[] => {
  const customers: Customer[] = [];
  const statuses: SubscriptionStatus[] = ['active', 'active', 'active', 'active', 'active', 'active', 'active', 'trial', 'trial', 'canceled', 'past_due'];
  const plans: PlanType[] = ['free', 'basic', 'basic', 'pro', 'pro', 'pro', 'enterprise'];

  for (let i = 0; i < 120; i++) {
    const firstName = firstNames[Math.floor(Math.random() * firstNames.length)];
    const lastName = lastNames[Math.floor(Math.random() * lastNames.length)];
    const company = companies[Math.floor(Math.random() * companies.length)] + (Math.random() > 0.5 ? ' Inc' : ' Ltd');
    const plan = plans[Math.floor(Math.random() * plans.length)];
    const status = statuses[Math.floor(Math.random() * statuses.length)];
    const segment = plan === 'enterprise' || (plan === 'pro' && Math.random() > 0.5) ? 'Enterprise' : 'SMB';
  ...
  return customers.sort((a, b) => b.registeredAt.getTime() - a.registeredAt.getTime());
};

export const generateSubscriptions = (customers: Customer[]): Subscription[] => {
  return customers
    .filter(c => c.plan !== 'free' && (c.status === 'active' || c.status === 'past_due'))
    .map((customer, i) => {
      const billingCycle = Math.random() > 0.4 ? 'monthly' : 'yearly';
      const multiplier = billingCycle === 'yearly' ? 10 : 1;
      const nextPayment = new Date();
      nextPayment.setDate(nextPayment.getDate() + (billingCycle === 'monthly' ? 30 : 365));
...
export const generateUserGrowthData = (): UserGrowthData[] => {
  ...
  let free = 200, trial = 30, basic = 40, pro = 25, enterprise = 5;
...
export const generateRecentSubscriptions = (): RecentSubscription[] => {
  ...
  const statuses: SubscriptionStatus[] = ['active', 'active', 'active', 'trial'];
...
export const generateCohortData = (): CohortData[] => {
  return [
    { cohort: 'Jan 2024', month0: 100, month1: 87, month2: 79, month3: 74, month4: 71, month5: 68 },
    { cohort: 'Feb 2024', month0: 100, month1: 89, month2: 82, month3: 78, month4: 75, month5: 0 },
    { cohort: 'Mar 2024', month0: 100, month1: 91, month2: 85, month3: 81, month4: 0, month5: 0 },
    { cohort: 'Apr 2024', month0: 100, month1: 88, month2: 83, month3: 0, month4: 0, month5: 0 },
    { cohort: 'May 2024', month0: 100, month1: 92, month2: 0, month3: 0, month4: 0, month5: 0 },
